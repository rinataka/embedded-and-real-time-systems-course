...
// Group: 13 (2022), 
// Work package 6, 
// Exercise 1: DC MOTOR CONTROLLER, Task 2&3
// Tinkercad: https://www.tinkercad.com/things/8lRtG5Cnc1e-copy-of-exerc612/editel?sharecode=kytftaebKFysp4xD3eZO_ryoZ_O7J7_C-9ovgKwWuKg
...
void setup() {
  ...
  */
  attachInterrupt(digitalPinToInterrupt(ENCA), ISR_readEncoder, RISING); //interrupt triggered when signal A is rising 
...
int getAction(int error){
  ...
  */
  ...
  u_out = error * kp; //use the error term to derive a control signal 𝑢 that is fed into the motor
...
void ISR_readEncoder(){
  ...
  */
  if(e){ //have to because if not the interrupt is triggered even if its standing still (tindercad bug)
  	b = digitalRead(ENCB);
  	if (b){ //if b is high, its turning counter clockwise
    	pos--;
  	}else{ //if b is low its turning clockwise
    	pos++;
  	}
  }
